...
namespace MinecraftInjector
{
    partial class MainWindow
    {
        ...
        { 
            ...
            if (!File.Exists(path))
            {
                SetStatus("DLL not found, your Antivirus might have deleted it.");
                goto done;
            }

            if (File.ReadAllBytes(path).Length < 10)
            {
                SetStatus("DLL broken (Less than 10 bytes)");
                goto done;
            }
            ...
            {
                SetStatus("Could not set permissions, try running the injector as admin.");
                goto done;
            }
            ...
            if (processes.Length == 0)
            {
                ...
                {
                    SetStatus("Failed to launch Minecraft for you. Please try starting it manually.");
                    goto done;
                }
                ...
                while (processes.Length == 0)
                {
                    if (++t > 200)
                    {
                        SetStatus("Minecraft launch took too long.");
                        return;
                    }
        ...
        private bool CheckGame()
        {
            ...
            {
                SetStatus("Minecraft does not appear to be installed. Please make sure you have the Windows edition of the game installed.");
                return false;
            }
